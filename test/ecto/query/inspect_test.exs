...
defmodule Ecto.Query.InspectTest do
  ...
  test "parameterized types" do
    query =
      from(x in Post,
        select: type(^"foo", ^Ecto.ParameterizedType.init(MyParameterizedType, param: :foo))
      )

    assert i(query) ==
             ~s<from p0 in Inspect.Post, select: type(^"foo", {:parameterized, {Ecto.Query.InspectTest.MyParameterizedType, :foo}})>
  end
  ...
  test "parameterized types after planner" do
    ...
    assert i(query) ==
             ~s<from p0 in Inspect.Post, select: type(^..., {:parameterized, {Ecto.Query.InspectTest.MyParameterizedType, :foo}})>
  end
